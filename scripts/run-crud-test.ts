...
async function main(): Promise<void> {
  ...
  if (!/^[A-Za-z0-9_]+_test$/.test(databaseName)) {
    throw new Error("CRUD test database name must end with _test");
  }
  ...
  run(bin("tsx"), ["drizzle/migration-enum-safety.test.ts"], env);
  run(bin("tsx"), ["src/db/seed.ts"], env);
  run(bin("tsx"), ["scripts/provision-revolut-accounts.ts"], env);
  run(bin("tsx"), ["scripts/provision-test-fx.ts"], env);
  run(bin("tsx"), ["src/lib/ledger/crud.e2e.test.ts"], env);
  run(bin("tsx"), ["src/lib/import/revolut/reversal-trashed-row.e2e.test.ts"], env);
  run(bin("tsx"), ["src/lib/import/revolut/reversal-restore-concurrency.e2e.test.ts"], env);
}
