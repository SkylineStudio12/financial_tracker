...
async function main(): Promise<void> {
  ...
  if (!/^[A-Za-z0-9_]+_test$/.test(databaseName)) {
    throw new Error("Management test database name must end with _test");
  }
  console.log("PASS management test-database sentinel (_test) and live-URL separation");
  ...
  run(bin("tsx"), ["src/lib/management/management.e2e.test.ts"], env);
}
