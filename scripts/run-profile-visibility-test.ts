...
async function main(): Promise<void> {
  ...
  if (!/^[A-Za-z0-9_]+_test$/.test(databaseName)) {
    throw new Error("Profile visibility test database name must end with _test");
  }
  console.log("PASS profile-visibility test-database sentinel (_test) and live-URL separation");
  ...
  run(bin("tsx"), ["src/lib/ledger/profile-visibility.e2e.test.ts"], env);
}
