...
async function main(): Promise<void> {
  ...
  if (!testRaw) throw new Error("TEST_DATABASE_URL is required for the reversal suite");
  ...
  if (!/^[A-Za-z0-9_]+_test$/.test(databaseName)) {
    throw new Error("Reversal test database name must end with _test");
  }
  ...
  run(bin("tsx"), ["src/lib/import/revolut/reversal.e2e.test.ts"], env);
}
