...
async function main(): Promise<void> {
  ...
  if (!testRaw) throw new Error("TEST_DATABASE_URL is required for the tax-config suite");
  ...
  if (!/^[A-Za-z0-9_]+_test$/.test(databaseName)) {
    throw new Error("Tax-config test database name must end with _test");
  }
  console.log("Test database guards passed: distinct from DATABASE_URL; name ends in _test.");
  ...
  run(bin("tsx"), ["src/lib/tax/config-service.e2e.test.ts"], env);
}
